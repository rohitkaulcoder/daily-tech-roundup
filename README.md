...
2. **Generate highlights** via Fireworks AI (OpenAI-compatible API, `deepseek-v4-flash`) — model configurable via the `FIREWORKS_MODEL` env var
...
| Secret | Description |
|--------|-------------|
| `FIREWORKS_API_KEY` | Fireworks AI API key (fireworks.ai) for highlight generation |
...
Optional env var: `FIREWORKS_MODEL` (default `accounts/fireworks/models/deepseek-v4-flash-0731`)
...
# Generate highlights (needs FIREWORKS_API_KEY)
...
~$2-4/month (Fireworks AI + Groq Whisper). Resend and GitHub Actions are free tier.
